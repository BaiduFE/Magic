 ...
 * @grammar {Boolean} options.autoHide 是否自动隐藏, 默认值为true。
 * @grammar {Boolean} options.hasCloseBtn 是否可以通过右上角X来关闭提示。默认值为false。
 * @grammar {Boolean} options.hasArrow 是否显示箭头。默认值为true。
 * @grammar {Boolean} options.autoBuild 自动检测需要提示的节点，通过attr值来判断。默认值为false。
 * @grammar {Number} options.offsetX 定位时的偏移量，X方向。
 * @grammar {Number} options.offsetY 定位时的偏移量，Y方向。
 * @grammar {Array} options.target 需要提示的节点。
 * @grammar {Function|String} options.content 自定义内容定制。若为Function,参数为Tangram对象(目标节点)。
 * @grammar {String} options.showEvent 提示显示的动作，默认值为mouseover,focus。
 * @grammar {String} options.hideEvent 提示隐藏的动作，默认值为mouseout,blur。
 * @grammar {String} options.position 设置tooltip的位置，值为top|bottom|left|right，默认值为bottom。
 * @grammar {Number|Percent} options.arrowPosition 设置arrow的位置，如果是上、下方位的话，都相对于左边的距离。如果是左、右方位的话，都相对于上面的距离。如果该值不存在，则自动计算相对于目标节点中间的位置。
 * @example 
