...
test("测试参数", function(){
    expect(5);
    stop();
    ua.importsrc('magic.setup.datePicker,baidu.dom.css,baidu.dom.attr,baidu.i18n.cultures.en-US,baidu.dom.trigger', function() {
        ua.loadcss(upath + "../../Calendar/magic.Calendar.css", function(){
            ...
            ok(baidu(datepicker.calendar.getElement("title").getElementsByTagName("span")[0]).css("display") != "none", "启用插件，默认显示年份的span标签");
            equals(datepicker.calendar.getElement("title").getElementsByTagName("select").length, 2, "启用插件，测试标题中的select个数");
            ok(baidu(datepicker.calendar.getElement("title").getElementsByTagName("span")[1]).css("display") != "none", "启用插件，默认显示月份的span标签");
...
test("测试基本操作", function(){
    ...
    ua.click(yearEl);
    ok(baidu(yearEl).css("display") == "none", "点击年份span标签，年份span标签隐藏");
    ok(baidu(yearSelect).css("display") != "none", "点击年份span标签，年份select标签显示");
    ...
    baidu(yearSelect).trigger('change');
    var yearEl = datepicker.calendar.getElement("title").getElementsByTagName("span")[0];
    var monthEl = datepicker.calendar.getElement("title").getElementsByTagName("span")[1];
    var yearSelect = datepicker.calendar.getElement("title").getElementsByTagName("select")[0];
    var monthSelect = datepicker.calendar.getElement("title").getElementsByTagName("select")[1];
    
    ok(baidu(yearEl).css("display") != "none", "选择年份后，年份span标签显示");
    ok(baidu(yearSelect).css("display") == "none", "选择年份后，年份select标签隐藏");
    ok(baidu(monthSelect).css("display") == "none", "选择年份后，月份select标签隐藏");
    ...
    ua.click(monthEl);
    ok(baidu(monthEl).css("display") == "none", "点击月份span标签，月份span标签隐藏");
    ok(baidu(monthSelect).css("display") != "none", "点击月份span标签，月份select标签显示");
    ...
    baidu(monthSelect).trigger('change');
    var yearEl = datepicker.calendar.getElement("title").getElementsByTagName("span")[0];
    var monthEl = datepicker.calendar.getElement("title").getElementsByTagName("span")[1];
    var yearSelect = datepicker.calendar.getElement("title").getElementsByTagName("select")[0];
    var monthSelect = datepicker.calendar.getElement("title").getElementsByTagName("select")[1];
    
    ok(baidu(monthEl).css("display") != "none", "选择月份后，年份span标签显示");
    ok(baidu(yearSelect).css("display") == "none", "选择月份后，年份select标签隐藏");
    ok(baidu(monthSelect).css("display") == "none", "选择月份后，月份select标签隐藏");
